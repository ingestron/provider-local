export function author(request) {
  if (
    request.apiVersion !== "ingestron.provider-authoring/v1" ||
    request.operation !== "initialise"
  )
    throw Error(
  ...
  if (
    ...
    !environments.length ||
    environments.some((e) => typeof e !== "string" || !/^\w[\w-]*$/.test(e))
  )
...
export function validateOutput(request) {
  ...
  for (const flow of Object.values(project.flows)) {
    if (
      !files[flow.directory + "/connector.json"] ||
      !files[flow.directory + "/singer_runtime.py"]
    )
